...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  // Run insight engine once; pass results into buildInsightContext to avoid recomputation
  const triggers = extractTriggers(entries);
  const patterns = detectPatterns(moodLogs);
  const context = buildInsightContext(entries, moodLogs, { triggers, patterns });

  // Persist detected triggers and patterns (fire-and-forget, parallel)
